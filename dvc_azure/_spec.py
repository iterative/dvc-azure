"""Temporary backport of version-aware adlfs spec."""
# type: ignore
# pylint: skip-file
import asyncio
import io
...
from adlfs.utils import close_container_client, close_service_client
...
async def get_blob_metadata(
    container_client, path, version_id: Optional[str] = None
):
...
class AzureBlobFileSystem(AsyncFileSystem):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.account_name = account_name or os.getenv(
            "AZURE_STORAGE_ACCOUNT_NAME"
        )
        self.account_key = account_key or os.getenv(
    ...
    def do_connect(self):
        ...
        try:
            if self.connection_string is not None:
                self.service_client = (
                    AIOBlobServiceClient.from_connection_string(
    ...
    def info(self, path, refresh=False, **kwargs):
        try:
            fetch_from_azure = (
                path and self._ls_from_cache(path) is None
            ) or refresh
        except Exception:
    ...
    async def _info(self, path, refresh=False, **kwargs):
        ...
        version_id = _coalesce_version_id(
            path_version_id, kwargs.get("version_id")
        )
        ...
        if out:
            return out[0]
        out = await self._ls(
            fullpath, invalidate_cache=invalidate_cache, **kwargs
        )
        ...
        **kwargs,
        ...
        if (container in ["", ".", "*", delimiter]) and (
            ...
            if (
                ...
                and not any(
                    match_blob_version(b, version_id)
                    for b in cache[target_path]
                )
            ):
                if container not in ["", delimiter]:
                    ...
                    async with self.service_client.get_container_client(
                        ...
                        blobs = cc.walk_blobs(
                            include=include, name_starts_with=path
                        )
                        ...
                        async for next_blob in blobs:
                            if depth in [0, 1] and path == "":
                                ...
                                async for blob_ in next_blob:
                                    if isinstance(
                                        blob_, BlobProperties
                                    ) or isinstance(blob_, BlobPrefix):
    ...
    async def _find(
        self, path, withdirs=False, prefix="", with_parent=False, **kwargs
    ):
    ...
    async def _rm_file(self, path, delimiter="/", **kwargs):
        ...
        try:
            ...
            container_name, path, _ = self.split_path(
    ...
    async def _cat_file(self, path, start=None, end=None, **kwargs):
        ...
        if end is not None:
            start = (
                start or 0
            )  # download_blob requires start if length is provided.
            length = end - start
        else:
            length = None
        container_name, path, version_id = self.split_path(path)
    ...
    async def _get_file(
        ...
        **kwargs,
        ...
        if os.path.isdir(lpath):
            return
        container_name, path, version_id = self.split_path(
            rpath, delimiter=delimiter
        )
        try:
            async with self.service_client.get_blob_client(
...
class AzureBlobFile(AbstractBufferedFile):
        ...
        if self.mode == "rb":
            if not hasattr(self, "details"):
                self.details = self.fs.info(
                    self.path, version_id=self.version_id
                )
            elif self.fs.version_aware and (
                (
                    self.version_id is None
                    and not self.details.get("is_current_version")
                )
                or (
    ...
    def close(self):
        ...
        asyncio.run_coroutine_threadsafe(
            close_container_client(self), loop=self.loop
        )
        super().close()
    ...
    def _get_chunks(
        self, data, chunk_size=1024**3
    ):  # Keeping the chunk size as 1 GB
        start = 0
        length = len(data)
        while start < length:
